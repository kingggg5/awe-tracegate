...
This is the shortest honest first run. It creates only synthetic local files,
does not call a model or network, and exits non-zero if the held bundle cannot
be replayed:
...
Expected result: `Gate v2 PASS`, `AWE review bundle: ready`, and stable receipt
hashes printed by the CLI. This demonstrates reproducibility of the verifier,
not agent quality or a production safety claim. For a real decision, replace
the synthetic artifacts with separately captured, consented evidence.
...
| CI cross-platform fixture | Ubuntu + Windows + macOS, Python 3.12 | Byte-identical `gate-v2`, comparison, explanation, and fixture artifacts | Canonical serialization is checked across the supported CI operating systems |

The cross-platform job is a reproducibility guard, not a model-quality result.
It compares the same synthetic inputs on all three hosted operating systems;
real agent trials still require an external pilot with consent.
...
The source implementation baseline is commit `a50f8a3`; documentation may have
later alignment commits. It is not an immutable release tag. A real pilot must use a non-sensitive
